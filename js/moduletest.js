...
function initHappyRobot() {
    fetch("assets/robot_happy.svg")
        .then((response) => response.text())
        .then((svgdata) => {
            document
                .querySelector("#i_robot_happy")
                .insertAdjacentHTML("afterbegin", svgdata);
            happyRobot();
        });
}

function initSadRobot() {
    fetch("assets/robot_sad.svg")
        .then((response) => response.text())
        .then((svgdata) => {
            document
                .querySelector("#i_robot_sad")
                .insertAdjacentHTML("afterbegin", svgdata);
            sadRobot();
        });
}
...
function happyRobot() {
    ...
    tlHappy.to("#robot", 0.2, { y: -100, scaleY: 1.05 }, "jumpUp");
    tlHappy.to(
        "#shadow",
        0.2,
    ...
    tlHappy.to("#robot", 0.2, { y: 0, scaleY: 1.05 }, "down");
...
function sadRobot() {
    ...
    tlSad.to("#eyes", 1, { y: 25, scale: 0.9 }, "eyesDown");

    tlSad.to("#left-eye", 0.7, { scaleY: 0.6, x: 1 }, "eyesDown");
    tlSad.to("#right-eye", 0.7, { scaleY: 0.6, x: -1 }, "eyesDown");
    ...
    tlSad.to("#eyes", 1, { y: 18, x: -13 }, "eyesLeft");

    tlSad.to("#right-eye", 0.7, { x: -9 }, "eyesLeft");

    tlSad.add("normal");

    tlSad.to("#eyes", 1, { y: 0, x: 0, scale: 1 }, "normal");

    tlSad.to("#left-eye", 0.7, { scaleY: 1, x: 0 }, "normal");

    tlSad.to("#right-eye", 0.7, { scaleY: 1, x: 0 }, "normal");
}
